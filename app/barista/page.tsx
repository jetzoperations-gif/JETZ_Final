...
export default function BaristaPage() {
    ...
    useEffect(() => {
        const checkAudioPermission = async () => {
            try {
                ...
                await audio.play()
                setSoundEnabled(true) // Browser allowed it!
            } catch (e) {
                console.log("Auto-play blocked, showing banner.") // Browser blocked it, show banner
    ...
    useEffect(() => {
        ...
        if (saved) {
            try {
                // Parse dates back to Date objects and ensure tokenId is present
                const parsed = JSON.parse(saved).map((n: any) => ({
                    ...n,
                    time: new Date(n.time),
                    tokenId: n.tokenId !== undefined ? n.tokenId : 0 // Default to 0 if not present (for older stored notifications)
                }))
        ...
        <MainLayout title="Cafe & Sales Log" role="staff">
            <div className="max-w-6xl mx-auto">
                ...
                <ActiveOrdersGrid
                    ...
                    onNewItem={(orderId, tokenId) => handleNewNotification(orderId, tokenId)}
                />
